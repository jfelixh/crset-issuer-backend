...
async function populateDbFromCsv(db: Database, filePath: string): Promise<void> {
  console.log(`Populating database with data from CSV file at: ${filePath}`);

  const insertStmt = db.prepare('INSERT OR REPLACE INTO credentialStatus (id, status) VALUES (?, ?)');
    const stream = fs.createReadStream(filePath, { encoding: 'utf8' }) // Ensure correct encoding
      .pipe(csv({
        headers: ['id', 'status'],
        skipLines: 1,
      }));
    ...
    stream.on("data", (row) => {
      ...
      // To check if rows are correctly inserted
     // console.log(`Inserting row: ID = ${credentialStatusId}, Status = ${status}`);
          ...
          db.all("SELECT status, id FROM credentialStatus", (err, rows) => {
            if (err) {
              ...
              if (rows.length === 0) {
                ...
                rows.forEach((row) => {
                  console.log(`✅ Found entry for ID ${row.id}: ${row.status}`);
                });
              }
